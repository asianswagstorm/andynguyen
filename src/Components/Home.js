...
class Home extends Component {
    ...
    if (!this.state.hasEmpty) {
      ...
      xhr.onreadystatechange = () => {
        ...
        if (xhr.status === 200) {
          ...
          popUpNotification(
            "error",
            "Sorry, there was an error while submitting the form."
          );
        }
      };
      xhr.send(data);
  ...
  setContactInfo = (event) => {
    ...
    const hasEmpty = Object.values(contactInfo).some(
    ...
    const language_buttons = languages.map((x, index) => {
      return (
        <li key={index}>
          <button
            onClick={() =>
              this.filter(x.languages !== "All" ? x.languages : "")
            }
        ...
        <section id="main">
            ...
            <div className="inner">
              <div className="heyThere">
                <h1 className="intro-andy">
                  <Wave
                    text="Hey! "
                    iterations={this.state.loaded === false ? 1 : 0}
                  />
                </h1>{" "}
                {/** redux function set timeout */}
          ...
          <section id="contact">
            <div className="social column">
              <h3>About Me</h3>
              <p id="about-me">
                My name is Andy Nguyen, a Montreal native software developer. I
                am a computer science graduate, who's always on the pursuit of
