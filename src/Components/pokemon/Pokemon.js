...
class Pokemon extends Component {
  constructor(props) {
    ...
    this.state = {
      name: "",
      pokemonIndex: this.props.urlParams.pokemonIndex,
      imageUrl: "",
      types: [],
      description: "",
      stats: {
  ...
  componentDidMount = () =>
    this.updatePokemon(this.props.urlParams.pokemonIndex);
  ...
  updatePokemon = async (pokemonIndex) => {
    try {
      ...
      const speciesData = await getPokeSpecies(pokemonIndex).then((response) =>
        response.json()
      );
      this.props.navigate(`/Pokemon/${pokemonIndex}`);

      this.setUpPokeState(pokemon, speciesData);
    } catch (error) {
      process.env.NODE_ENV.trim() !== "production" &&
        console.log("Invalid pokemon ID"); //front end
  ...
  setUpPokeState = async (pokemon, speciesData) => {
    ...
    const habitat =
      speciesData.habitat != null
        ? this.capitalize_firstLetter(speciesData.habitat.name)
        : "Not Available";
    ...
    const stage2 =
      data_json.evolves_to[0] != null
        ? this.capitalize_firstLetter(data_json.evolves_to[0].species.name)
        : "None";
    const stage2ID =
      data_json.evolves_to[0] != null
        ? data_json.evolves_to[0].species.url.split("/")[6]
        : 0;
    const stage3 =
      data_json.evolves_to[0] != null
        ? data_json.evolves_to[0].evolves_to[0] != null
          ? this.capitalize_firstLetter(
    ...
    const stage3ID =
      data_json.evolves_to[0] != null
        ? data_json.evolves_to[0].evolves_to[0] != null
          ? data_json.evolves_to[0].evolves_to[0].species.url.split("/")[6]
          : 0
        : 0;
  ...
  render = () => {
    ...
    const pokeIndex = parseInt(this.props.urlParams.pokemonIndex, 10);
            ...
            <div className="card">
              <div className="card">
                <div className="pokemon__info card-header">
                  <div className="poke_index row">
                    Pokedex #{this.state.pokemonIndex}
                  </div>

                  <div className="prev__next__poke row">
                    <button
                      id="toggle__pokemon__btn"
                      onClick={() =>
                        this.updatePokemon(pokeIndex >= 2 ? pokeIndex - 1 : 807)
                      }
                      className="previous_pokemon"
                    >
                      {" "}
                      prev
                    </button>
                    <button
                      id="toggle__pokemon__btn"
                      onClick={() =>
                        this.updatePokemon(pokeIndex < 807 ? pokeIndex + 1 : 1)
                      }
                ...
                <div className="pokemon_card_body">
                  ...
                  <div className="pokemon_profile row">
                    <div className="pokemon_profile_left col-sm-7">
                      <div className="poke_left_content">{leftProfile}</div>
                    </div>
                    <div className="pokemon_profile_right col-sm-5">
                      <div className="poke_right_content">{rightProfile}</div>
                    </div>
                  </div>
                </div>
                <div className="pokemon_card_body">
